...
with col1:
    st.space()
    if st.session_state.get("jenis_kelamin") == 1:
        st.image("images/maleprofil.png", width=90)
    else :
        st.image("images/femaleprofil.png", width=90)

with col2:
    st.subheader(st.session_state.nama)
    st.caption(f"NIP: {st.session_state.nip}")
    st.caption(f"{st.session_state.jabatan} • {st.session_state.unit_kerja}")
